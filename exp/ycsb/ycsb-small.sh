...
do_load() {
	...
	cd ${ycsb_dir} 
	./bin/ycsb load redis -s -P ${ycsb_workloads_dir}/${workload} -p redis.host=127.0.0.1 -p redis.port=5556 -p redis.timeout=10000000  -threads 10 > ${output_file_ycsb} 2>&1
	cd -
...
do_bulk_run() {
	...
	cd ${ycsb_dir} 
	./bin/ycsb run redis -s -P ${ycsb_workloads_dir}/${workload} -p redis.host=127.0.0.1 -p redis.port=5556 -p redis.timeout=10000000 -threads 10 > ${output_file_ycsb} 2>&1
	cd -
...
prepare_workloads() {
	cp ${bigkv_dir}/exp/ycsb/ycsb-example-small ${ycsb_workloads_dir}
}
...
do
	...
	do
		...
		do
			...
			for workload in ycsb-example-small
			do
				do_load
				do_bulk_run
			done
