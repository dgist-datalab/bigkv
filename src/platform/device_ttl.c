	...
	list_for_each_node_safe(dev->flying_seg_list, cur, next) {
		...
		if (seg->state & SEG_STATE_COMMITTED) {
			...
			if (seg->state & SEG_STATE_IDX)
				...
				ss->ttl_lnode = list_insert(dev->ttl_fifo[ss->ttl], ss);
		}
	}
	return 0;
}
...
bool dev_need_gc(struct handler *hlr, struct dev_abs *dev) {
...
static struct segment *select_gc_victim_seg(struct handler *hlr, struct dev_abs *dev) {
	...
	list *fifo_list;
	int fifo_idx = 0;
	uint64_t min_creation_time = get_cur_sec();

	for (int i = 0; i < NR_TTL_GROUPS; i++) {
		seg = (struct segment *)dev->ttl_fifo[i]->head->data;
	...
	if (idx_seg->creation_time < min_creatio_time) {
		seg = idx_seg;
	} else {
...
int dev_read_victim_segment(struct handler *hlr, struct dev_abs *dev, struct gc *gc) {
...
void reap_gc_segment(struct handler *hlr, struct dev_abs *dev, struct gc *gc) {
